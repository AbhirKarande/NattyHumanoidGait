...
class DeepDeterministicPolicyGradient(object):
  ...
  def create_variables(self):
    ...
    with tf.name_scope("estimate_future_rewards"):
      ...
      self.next_state_mask = tf.placeholder(tf.float32, (None,), name="next_state_mask")#s")
    ...
    with tf.name_scope("compute_pg_gradients"):
      self.w_id=tf.placeholder(tf.float32, (None,), name="w_id")
      self.w   =tf.reshape(self.w_id, [32,1])
      #self.ww=tf.shape(self.w_id)
      # compute gradients for critic network
      self.temp_diff        = self.value_outputs[:,0] - self.future_rewards#TD-error
      #self.temp             = tf.diag(tf.matmul(tf.square(self.temp_diff), self.w_id, False, True))
      #tf.transpose
      #self.mean_square_loss = tf.reduce_mean(tf.square(self.temp_diff))#tf.square(self.temp_diff))
      self.tem=tf.reshape(tf.square(self.temp_diff),[32,1])
      self.temp=tf.matmul(tf.transpose(self.tem), self.w)/self.batch_size
      #self.dim=tf.shape(self.temp)
      #self.temp=tf.reshape(self.temp,[1,1])
      self.mean_square_loss = tf.reduce_mean(self.temp)
      #self.mean_square_loss = tf.reduce_mean(tf.square(self.temp_diff))#self.temp/self.batch_size#tf.reduce_mean(self.temp)
      self.critic_reg_loss  = tf.reduce_sum(tf.stack([tf.reduce_sum(tf.square(x)) for x in critic_network_variables]))
  ...
  def updateModel(self, recorder):

    # not enough experiences yet
    if self.replay_buffer.count() < self.batch_size*20:
      return

    batch, w_id, eid= self.replay_buffer.getBatch(self.batch_size)
    ...
    for i in range(32):
      recorder[e_id[i]%10000]=recorder[e_id[i]%10000]+1
    ...
    critic_loss, _, delta = self.session.run([
      self.critic_loss,
      self.train_op,
    ...
    self.error=delta
    # print temp1, self.train_iteration
    ...
    #self.replay_memory.update_priority(self.e_id, self.error)
    #    if self.time_step % 100 ==0:
    #        self.replay_memory.rebalance()
    self.replay_buffer.update_priority(e_id, self.error)
